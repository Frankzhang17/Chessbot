...
public class Board {
    ...
    public int[][] rookMoves(int row, int col) {
        ...
        for (int d = 0; d < 4; d++) {
            for (int step = 1; step <= 7; step++) {
                ...
                if (get(row, col) > 0 && get(newRow, newCol) > 0) break;
                if (get(row, col) < 0 && get(newRow, newCol) < 0) break;
    ...
    public boolean makeMove(int fromRow, int fromCol, int toRow, int toCol) {
        ...
        if      (piece == PAWN   || piece == -PAWN)   validMoves = pawnMoves(fromRow, fromCol);
        else if (piece == ROOK   || piece == -ROOK)   validMoves = rookMoves(fromRow, fromCol);
        else if (piece == KNIGHT || piece == -KNIGHT) validMoves = knightMoves(fromRow, fromCol);
        else if (piece == BISHOP || piece == -BISHOP) validMoves = bishopMoves(fromRow, fromCol);
        else if (piece == QUEEN  || piece == -QUEEN)  validMoves = queenMoves(fromRow, fromCol);
        else if (piece == KING   || piece == -KING)   validMoves = kingMoves(fromRow, fromCol);
        ...
        for (int i = 0; i < validMoves.length; i++) {
            ...
            if (validMoves[i][0] == toRow && validMoves[i][1] == toCol) {
                if (leavesKingInCheck(fromRow, fromCol, toRow, toCol)) return false;

                boolean isEnPassant = (piece == PAWN || piece == -PAWN)
    ...
    public boolean isInCheck(boolean white) {
        ...
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                if (white  && get(row, col) ==  KING) { kingRow = row; kingCol = col; }
                if (!white && get(row, col) == -KING) { kingRow = row; kingCol = col; }
            }
        }
        ...
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                ...
                if (white && piece < 0 || !white && piece > 0) {
                    if      (piece == PAWN   || piece == -PAWN)   enemyMoves = pawnMoves(row, col);
                    else if (piece == ROOK   || piece == -ROOK)   enemyMoves = rookMoves(row, col);
                    else if (piece == KNIGHT || piece == -KNIGHT) enemyMoves = knightMoves(row, col);
                    else if (piece == BISHOP || piece == -BISHOP) enemyMoves = bishopMoves(row, col);
                    else if (piece == QUEEN  || piece == -QUEEN)  enemyMoves = queenMoves(row, col);
                    else if (piece == KING   || piece == -KING)   enemyMoves = kingMovesOnly(row, col);
                }

                for (int i = 0; i < enemyMoves.length; i++) {
                    if (enemyMoves[i][0] == kingRow && enemyMoves[i][1] == kingCol) return true;
                }
            }
        }
        ...
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                ...
                for (int i = 0; i < moves.length; i++) {
                    if (!leavesKingInCheck(row, col, moves[i][0], moves[i][1])) return false;
                }
            }
        }
    ...
    public boolean isThreefoldRepetition() {
        ...
        for (int[] snapshot : boardHistory) {
            if (Arrays.equals(snapshot, current)) count++;
        }
        return count >= 3;
    }
